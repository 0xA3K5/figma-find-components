...
import { IComponent, IComponentInstance, ReplaceInstances } from '../types';

interface Props {
  options: Record<string, IComponent[]>
  checkedInstances: IComponentInstance[]
}

export default function Select({ options, checkedInstances }: Props): JSX.Element {
  ...
  const handleSelect = (event: JSX.TargetedEvent<HTMLSelectElement, Event>) => {
    ...
    if (!comp) {
      ...
      emit<ReplaceInstances>('REPLACE_INSTANCES', ({ instances: checkedInstances, replaceWith: comp }));
